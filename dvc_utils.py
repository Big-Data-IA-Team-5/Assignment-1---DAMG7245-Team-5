...
class DVCUtils:
    ...
    def show_status(self):
        ...
        if result and result.returncode == 0:
            ...
            print(f"❌ Error checking status: {result.stderr if result else 'Unknown error'}")
    ...
    def show_metrics(self, json_format=False):
        ...
        if result and result.returncode == 0:
            ...
            print(f"❌ Error retrieving metrics: {result.stderr if result else 'Unknown error'}")
    ...
    def compare_metrics(self, revision="HEAD~1"):
        ...
        result = self.run_command(
            ["dvc", "metrics", "diff", revision], 
            f"Comparing with {revision}..."
        )
        ...
        if result and result.returncode == 0:
            ...
            print(f"❌ Error comparing metrics: {result.stderr if result else 'Unknown error'}")
