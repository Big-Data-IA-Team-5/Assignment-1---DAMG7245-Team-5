...
try:
    ...
    print(
        "Make sure you're running from the project root directory or the google_ai folder."
    )
    sys.exit(1)
...
class Lab7GoogleAIWorkflow:
    ...
    def _load_config(self, config_path):
        ...
        if not config_path_obj.exists():
            logger.warning(
                f"Config file not found: {config_path}. Creating default config."
            )
    ...
    def _create_default_config(self, config_path):
        ...
        logger.warning(
            "Please update the configuration with your Google Cloud credentials!"
        )
    ...
    def extract_pages(self, pdf_path, page_numbers=None, random_pages=None, seed=None):
        ...
        logger.info(
            f"PDF has {pdf_info['total_pages']} pages, size: {pdf_info['file_size_mb']:.2f} MB"
        )
        ...
        if random_pages:
            ...
            temp_pdf_path, selected_pages = extractor.extract_random_pages(
                ...
                seed=seed
                or self.config.get("page_selection", {}).get("random_seed", 42),
            )
            # Convert to proper types
            temp_pdf_path = str(temp_pdf_path)
            selected_pages = list(selected_pages) if selected_pages else []
            selected_pages_1indexed = [
                int(p) + 1 for p in selected_pages
            ]  # Convert to 1-indexed
        else:
            # Extract specific pages
            if not page_numbers:
                page_numbers = self.config.get("page_selection", {}).get(
                    "default_pages", [5, 12]
                )
        ...
        logger.info(
            f"Created temporary PDF with pages {selected_pages_1indexed}: {temp_pdf_path}"
        )
    ...
    def process_with_google_ai(self, temp_pdf_path):
        ...
        if not project_id or not processor_id:
            raise ValueError(
                "Google Cloud project_id and processor_id must be configured!"
            )
    ...
    def _get_parsed_data_for_pages(self, parsed_dir, pages_processed):
        ...
        logger.info(
            f"Parsed data page-specific analysis: {parsed_summary['tables_found']} tables, {parsed_summary['text_length']} characters"
        )
        return parsed_summary

    def _create_page_comparison(
        self, google_ai_summary, parsed_summary, pages_processed
    ):
        ...
        comparison = {
            ...
            "comparison_results": {
                "pages_analyzed": pages_processed,
                "text_length_ratio": (
                    google_ai_summary["total_text_length"]
                    / max(parsed_summary["text_length"], 1)
                ),
                "table_detection": {
        ...
        if "page_specific_data" in parsed_summary:
            for page in pages_processed:
                page_data = parsed_summary["page_specific_data"].get(
                    page, {"tables": 0, "text_length": 0}
                )
        ...
        if (
            google_ai_summary["tables_found"] == 0
            and parsed_summary["tables_found"] == 0
        ):
            comparison["analysis"].append(
                f"Neither Google AI nor your parsed data detected tables in pages {pages_processed}"
            )
        elif (
            google_ai_summary["tables_found"] == 0
            and parsed_summary["tables_found"] > 0
            ...
            comparison["analysis"].append(
                f"Your parsed data detected {parsed_summary['tables_found']} tables in pages {pages_processed}, Google AI detected none"
            )
        elif (
            google_ai_summary["tables_found"] > 0
            and parsed_summary["tables_found"] == 0
            ...
            comparison["analysis"].append(
                f"Google AI detected {google_ai_summary['tables_found']} tables in pages {pages_processed}, your parsed data detected none"
            )
        else:
            comparison["analysis"].append(
                f"Both detected tables: Google AI ({google_ai_summary['tables_found']}) vs Your parsed data ({parsed_summary['tables_found']})"
            )

        text_ratio = comparison["comparison_results"]["text_analysis"]["ratio"]
        if text_ratio < 0.5:
            comparison["analysis"].append(
                f"Google AI extracted {text_ratio*100:.1f}% of your parsed data's text from same pages - may indicate different extraction methods"
            )
        elif text_ratio > 1.5:
            comparison["analysis"].append(
                f"Google AI extracted {text_ratio*100:.1f}% more text than your parsed data from same pages"
            )
        else:
            comparison["analysis"].append(
                f"Text extraction similar: Google AI vs your parsed data ratio = {text_ratio:.2f}"
            )
    ...
    def compare_with_parsed_data(self, google_results, original_pdf, selected_pages):
        ...
        """
        logger.info(
            "Comparing Google AI results with your parsed data using page-specific matching..."
        )
        ...
        if parsed_result_dirs:
            ...
            comparison = self._create_page_comparison(
                google_ai_summary, parsed_summary, selected_pages
            )
        ...
        comparison_file = (
            self.comparison_dir / f"parsed_data_vs_google_ai_comparison.json"
        )
    ...
    def generate_report(self, workflow_results):
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_path = (
            self.final_report_dir / f"lab7_comprehensive_report_{timestamp}.md"
        )
...
"""
        ...
        if "saved_files" in workflow_results["parsed_results"]:
            report_content += "\n## Generated Files\n"
            for file_type, file_path in workflow_results["parsed_results"][
                "saved_files"
            ].items():
...
"""
    ...
    def run_complete_workflow(
        self, pdf_path, page_numbers=None, random_pages=None, seed=None
    ):
        """
        ...
        try:
            ...
            temp_pdf, selected_pages = self.extract_pages(
                pdf_path, page_numbers, random_pages, seed
            )
            ...
            comparison_results = self.compare_with_parsed_data(
                parsed_results, pdf_path, selected_pages
            )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="data/lab7_results", help="Output directory for results"
    )
    ...
    if not args.pages and not args.random:
        print(
            "Using default pages [5, 12]. Use --pages or --random to specify different selection."
        )
