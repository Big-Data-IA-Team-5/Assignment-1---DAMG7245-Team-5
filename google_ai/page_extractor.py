...
class PDFPageExtractor:
    ...
    def _load_pdf(self):
        ...
        try:
            ...
            logger.info(f"Loaded PDF: {self.pdf_path.name} with {self.total_pages} pages")
    ...
    def extract_pages(self, page_numbers, output_path=None):
        ...
        if invalid_pages:
            raise ValueError(f"Invalid page numbers: {invalid_pages}. PDF has {self.total_pages} pages (0-{self.total_pages-1})")
        
        # Generate output path if not provided
        if output_path is None:
            pages_str = "_".join(map(str, [p+1 for p in page_numbers]))  # Convert to 1-indexed for filename
            output_path = self.pdf_path.parent / "temp_pdfs" / f"{self.pdf_path.stem}_pages_{pages_str}.pdf"
        ...
        try:
            ...
            logger.info(f"Created temporary PDF with {len(page_numbers)} pages: {output_path}")
    ...
    def extract_random_pages(self, num_pages=2, seed=None):
        ...
        if num_pages > self.total_pages:
            raise ValueError(f"Cannot extract {num_pages} pages from a {self.total_pages}-page PDF")
        
        # Select random pages (0-indexed)
        selected_pages = sorted(random.sample(range(self.total_pages), num_pages))
        
        logger.info(f"Randomly selected pages: {[p+1 for p in selected_pages]} (1-indexed)")
...
if __name__ == "__main__":
    ...
    try:
        ...
        temp_pdf_random, selected_pages = extractor.extract_random_pages(num_pages=2, seed=42)
        print(f"Random pages {[p+1 for p in selected_pages]} extracted to: {temp_pdf_random}")
