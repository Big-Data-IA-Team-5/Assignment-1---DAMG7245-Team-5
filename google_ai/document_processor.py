...
class GoogleDocumentAIProcessor:
    ...
    def __init__(self, project_id: str, location: str = "us", processor_id: Optional[str] = None):
        """
        ...
        if not GOOGLE_AI_AVAILABLE:
            raise ImportError("Google Document AI client library not available. Install with: pip install google-cloud-documentai")
        ...
        try:
            ...
            if self.processor_id:
                ...
                for processor in processors:
                    if "general" in processor.type_.lower() or "document" in processor.type_.lower():
                        self.processor_name = processor.name
                        self.processor_id = processor.name.split("/")[-1]
                        break
                
                if not self.processor_name:
                    raise ValueError("No suitable processor found. Please specify a processor_id.")
            
            logger.info(f"Initialized Google Document AI client with processor: {self.processor_id}")
            
        except Exception as e:
            logger.error(f"Failed to initialize Google Document AI client: {e}")
            raise
    
    def process_pdf(self, pdf_path: str, output_dir: Optional[str] = None) -> Tuple[Dict, str]:
        """
        ...
        try:
            ...
            logger.info(f"Google Document AI processing completed. Output saved to: {output_json}")
    ...
    def _document_to_dict(self, document) -> Dict:
        ...
        for page_idx, page in enumerate(document.pages):
            ...
            for paragraph in page.paragraphs:
                page_dict["paragraphs"].append(self._layout_to_dict(paragraph.layout, "paragraph"))
            ...
            for table_idx, table in enumerate(page.tables):
                table_dict = {
                    ...
                    "header_rows": [],
                    "body_rows": [],
                    "layout": self._layout_to_dict(table.layout, "table") if table.layout else None
                }
                
                # Process header rows
                for row in table.header_rows:
                    table_dict["header_rows"].append(self._process_table_row(row, document.text))
                
                # Process body rows
                for row in table.body_rows:
                    table_dict["body_rows"].append(self._process_table_row(row, document.text))
            ...
            for form_field in page.form_fields:
                field_dict = {
                    "field_name": self._extract_text_from_layout(form_field.field_name, document.text) if form_field.field_name else "",
                    "field_value": self._extract_text_from_layout(form_field.field_value, document.text) if form_field.field_value else "",
                    "name_layout": self._layout_to_dict(form_field.field_name, "form_field_name") if form_field.field_name else None,
                    "value_layout": self._layout_to_dict(form_field.field_value, "form_field_value") if form_field.field_value else None
                }
    ...
    def _layout_to_dict(self, layout, element_type: str) -> Dict:
        ...
        layout_dict = {
            ...
            "bounding_poly": None,
            "orientation": layout.orientation if hasattr(layout, 'orientation') else None
        }
        ...
        if hasattr(layout, 'bounding_poly') and layout.bounding_poly:
            layout_dict["bounding_poly"] = {
                "vertices": [{"x": v.x, "y": v.y} for v in layout.bounding_poly.vertices]
            }
    ...
    def _process_table_row(self, row, document_text: str) -> List[Dict]:
        ...
        for cell in row.cells:
            cell_dict = {
                "text": self._extract_text_from_layout(cell.layout, document_text) if cell.layout else "",
                "row_span": cell.row_span,
                "col_span": cell.col_span,
                "layout": self._layout_to_dict(cell.layout, "table_cell") if cell.layout else None
            }
        ...
        try:
            ...
            for processor in processors:
                processor_list.append({
                    ...
                    "processor_version_aliases": list(processor.processor_version_aliases)
                })
...
def process_pdf_with_google_ai(pdf_path: str, project_id: str, processor_id: str,
                             location: str = "us", output_dir: Optional[str] = None) -> Dict:
    """
...
if __name__ == "__main__":
    ...
    try:
        if GOOGLE_AI_AVAILABLE:
            ...
            print("Google Document AI library not available. Install with: pip install google-cloud-documentai")
