...
class SecureCredentialManager:
    ...
    def find_credentials(self, config_file=None):
        ...
        if Path(project_creds).exists():
            ...
            if self._validate_credentials_file(project_creds):
                ...
                logger.warning("   Consider using environment variables for better security")
        ...
        if config_file and Path(config_file).exists():
            try:
                ...
                config_creds = config.get("google_document_ai", {}).get("credentials_path")
    ...
    def _validate_credentials_file(self, creds_path):
        ...
        try:
            ...
            if missing_fields:
                logger.error(f"Invalid credentials file {creds_path}. Missing fields: {missing_fields}")
                return False
            ...
            if permissions not in ['600', '644']:
                logger.warning(f"⚠️  Credentials file has permissions {permissions}. Consider setting to 600 for security.")
