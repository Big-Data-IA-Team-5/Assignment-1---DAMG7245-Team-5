...
class GoogleAIResultParser:
    ...
    def _load_json(self):
        ...
        try:
            ...
            logger.info(f"Loaded Google AI results: {len(self.pages)} pages from {self.json_path.name}")
    ...
    def get_document_info(self):
        ...
        return {
            ...
            "has_form_fields": any(len(page.get('form_fields', [])) > 0 for page in self.pages),
        ...
        for page_idx, page in enumerate(self.pages):
            ...
            for table_idx, table in enumerate(tables):
                ...
                if table_data['all_rows']:
                    table_data['col_count'] = max(len(row) for row in table_data['all_rows'])
        ...
        for table in tables:
            ...
            if table['all_rows']:
                ...
                if table['header_rows']:
                    columns = table['header_rows'][0] if table['header_rows'][0] else [f"Col_{i}" for i in range(table['col_count'])]
                ...
                for row in data_rows:
                    ...
                    normalized_rows.append(normalized_row[:max_cols])  # Truncate if too long
                
                try:
                    df = pd.DataFrame(normalized_rows, columns=columns)
                    dataframes.append((table_id, df))
                except Exception as e:
                    logger.warning(f"Failed to create DataFrame for table {table_id}: {e}")
