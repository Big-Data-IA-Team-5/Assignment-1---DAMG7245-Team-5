    ...
    for page_layout in layout_data:
        ...
        for block in page_layout["blocks"]:
            ...
            if block_type in ["Text", "Title"]:
                ...
                routing_info["action"] = (
                    "Extract structured data using Camelot stream mode"
                )
...
def demonstrate_reading_order(layout_data):
    ...
    for page_layout in layout_data:
        ...
        sorted_blocks = sorted(
            text_blocks,
            key=lambda x: (x["bounding_box"]["y1"], x["bounding_box"]["x1"]),
        )
        reading_sequence = []
...
def save_layout_detection_results(
    layout_data, routing_results, reading_order, pdf_name, out_dir
):
...
def main():
    parser = argparse.ArgumentParser(
        description="Lab 3: Layout Detection for Complex Pages"
    )
    parser.add_argument(
    ...
    for pdf_path in pdf_files:
        ...
        summary = save_layout_detection_results(
            layout_data, routing_results, reading_order, pdf_path.name, out_dir
        )
