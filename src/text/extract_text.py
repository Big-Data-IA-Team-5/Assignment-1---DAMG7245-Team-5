...
def extract_text_with_pdfplumber(pdf_path):
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page_num, page in enumerate(pdf.pages):
            ...
            if text and len(text.strip()) > 50:
                ...
                logger.info(f"Page {page_num + 1}: Extracted {len(text)} characters with pdfplumber")
            else:
                # No text found - likely a scanned page, apply OCR fallback
                logger.warning(f"Page {page_num + 1}: No text extracted, applying OCR fallback")
                try:
                    ...
                    if ocr_text and len(ocr_text.strip()) > 10:
                        ...
                        logger.error(f"Page {page_num + 1}: OCR failed to extract meaningful text")
    ...
    # Log summary
    logger.info(f"Extraction complete: {len(extracted_pages)} pages processed, {len(ocr_pages)} pages required OCR")
...
def save_extracted_data(pages_data, ocr_pages, pdf_name, out_dir):
    ...
    for page in pages_data:
        ...
        with open(words_file, 'w', encoding='utf-8') as f:
            json.dump({
                ...
                'extraction_method': page['extraction_method'],
                'word_count': len(page['words_with_bbox']) if page['words_with_bbox'] else 0,
    ...
    ocr_log = {
        ...
        'summary': {
            'successful_pdfplumber': len([p for p in pages_data if p['extraction_method'] == 'pdfplumber']),
            'successful_ocr': len([p for p in pages_data if p['extraction_method'] == 'tesseract_ocr']),
            'failed_extraction': len([p for p in pages_data if p['extraction_method'] == 'failed'])
        }
    }
    ...
    with open(summary_report, 'w', encoding='utf-8') as f:
        ...
        f.write(f"- **Successfully Extracted (pdfplumber):** {ocr_log['summary']['successful_pdfplumber']}\n")
        f.write(f"- **Required OCR:** {ocr_log['summary']['successful_ocr']}\n")
        f.write(f"- **Failed Extraction:** {ocr_log['summary']['failed_extraction']}\n\n")
        
        if ocr_pages:
            f.write(f"## Pages Requiring OCR\n")
            f.write(f"The following pages were scanned/image-based and required OCR fallback:\n")
