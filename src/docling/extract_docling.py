...
def analyze_docling_structure(doc):
    ...
    if hasattr(doc, "main_text"):
        ...
        analysis["reading_order_analysis"] = {
            "reading_order_preserved": True,
            "text_preview": text_preview,
            "logical_flow_detected": True,
        }
    return analysis
...
def export_docling_formats(doc, pdf_name, out_dir):
            ...
            json_content = {
                ...
                "content": {
                    ...
                    "text_preview": (
                        ...
                        if hasattr(doc, "main_text")
                        and doc.main_text
                        and len(doc.main_text) > 1000
                        else (doc.main_text if hasattr(doc, "main_text") else "")
                    ),
                },
                "tables": [],
                "structure": {
                    "has_tables": hasattr(doc, "tables")
                    and doc.tables is not None
                    and len(doc.tables) > 0,
                    "table_count": (
                        len(doc.tables) if hasattr(doc, "tables") and doc.tables else 0
                    ),
                    "has_equations": hasattr(doc, "equations")
                    and doc.equations is not None
                    and len(doc.equations) > 0,
                    "equation_count": (
                        len(doc.equations)
                        if hasattr(doc, "equations") and doc.equations
                        else 0
    ...
    with open(docling_root / "comparison.txt", "w", encoding="utf-8") as f:
        f.write(
            "See output.md and output.json for Docling results. Compare with traditional pipeline outputs in text/ and tables/."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Lab 4: Docling Advanced PDF Understanding"
    )
    parser.add_argument(
    ...
    for pdf_path in pdf_files:
        ...
        print(
            f"Tables detected: {docling_analysis.get('table_analysis', {}).get('total_tables', 0)}"
        )
        print(
            f"Formulas detected: {docling_analysis.get('formula_detection', {}).get('total_formulas', 0)}"
        )
        print(
            f"Reading order preserved: {docling_analysis.get('reading_order_analysis', {}).get('reading_order_preserved', False)}"
        )
