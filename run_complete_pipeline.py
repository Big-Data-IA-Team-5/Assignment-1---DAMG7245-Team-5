...
def validate_lab_outputs(doc_id, output_dir):
    ...
    validations["lab1"] = {
        ...
        "status": (
            all(
                [
                    (text_dir / f).exists()
                    for f in ["extracted_text.txt", "_ocr_pages.json"]
                ]
            )
    ...
    validations["lab3"] = {
        ...
        "status": (
            all(
                [
                    (layout_dir / f).exists()
                    for f in ["layout_blocks.json", "layout_aware_extraction.json"]
                ]
            )
            and (layout_dir / "blocks").exists()
    ...
    validations["lab4"] = {
        ...
        "status": (
            all(
                [
                    (docling_dir / f).exists()
                    for f in ["output.md", "output.json", "comparison.txt"]
                ]
            )
...
def generate_pipeline_report(
    output_dir, lab_results, successful_pdfs, failed_pdfs, execution_info
):
...
def create_metadata_from_docling(
...
- Processed with Docling's state-of-the-art PDF understanding"""
    ...
    try:
        ...
        logger.info(
            f"[LAB 5] Created metadata file with {len(metadata_records)} records: {metadata_file}"
        )
        return True
    except Exception as e:
...
Examples:
  ...
  %(prog)s --out results --hybrid-tables --company "My Company"
    ...
    parser.add_argument(
        "--raw-dir", default="data/raw", help="Raw data directory (default: data/raw)"
    )
    parser.add_argument(
    ...
    for i, pdf_path in enumerate(pdf_files, 1):
        ...
        logger.info(
            f"Document metadata: ID={doc_id}, Company={company}, Year={fiscal_year}"
        )
        ...
        lab1_success = run_command(
            [
                py_exec,
                str(base_dir / "src" / "text" / "extract_text.py"),
                "--in",
                str(pdf_path),
                "--out",
        ...
        table_cmd = [
            py_exec,
            str(base_dir / "src" / "tables" / "extract_tables.py"),
            "--in",
            str(pdf_path),
            "--out",
            str(unified_output_dir),
        ...
        lab3_success = run_command(
            [
                py_exec,
                str(base_dir / "src" / "layout" / "extract_layout.py"),
                "--in",
                str(pdf_path),
                "--out",
                str(unified_output_dir),
        ...
        if not args.skip_docling:
            ...
            lab4_success = run_command(
                [
                    py_exec,
                    str(base_dir / "src" / "docling" / "extract_docling.py"),
                    "--in",
                    str(pdf_path),
                    "--out",
                    str(unified_output_dir),
        ...
        if not metadata_file.exists():
            ...
            metadata_file = (
                unified_output_dir / "metadata" / f"{doc_id}_docling_summary.json"
            )
            if metadata_file.exists():
                logger.info(
                    f"[LAB 6] Using Docling-generated metadata: {metadata_file}"
                )
            else:
                metadata_file = unified_output_dir / "metadata" / f"{doc_id}.jsonl"
        ...
        if metadata_file.exists():
            lab6_success = run_command(
                [
                    py_exec,
                    str(base_dir / "src" / "formats" / "convert_formats.py"),
                    "--in",
                    str(metadata_file),
                    "--out",
                    str(unified_output_dir),
    ...
    logger.info(
        f"Success rate: {len(successful_pdfs) / len(pdf_files) * 100 if pdf_files else 0:.1f}%"
    )
    ...
    for lab_num in range(1, 7):
        ...
        successes = sum(
            1 for results in lab_results.values() if results.get(lab_key, False)
        )
        total = len(lab_results)
        percentage = (successes / total * 100) if total > 0 else 0
        logger.info(
            f"   Lab {lab_num}: {successes}/{total} successful ({percentage:.1f}%)"
        )
    ...
    summary_data = {
        ...
        "lab_summary": {
            f"lab_{i}": {
                "successful": sum(
                    1
                    for results in lab_results.values()
                    if results.get(f"lab{i}", False)
                ),
                "total": len(lab_results),
                "success_rate": (
                    sum(
    ...
    report_file = generate_pipeline_report(
        output_dir, lab_results, successful_pdfs, failed_pdfs, execution_info
    )
    if report_file:
